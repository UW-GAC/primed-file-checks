...
for (i in 1:length(vars)) {
  ...
  min <- vars_df[i, 2]
  print(var_name)
  print (this_table %>% summarise(
    n = n(), 
    qc_pass = sum(var_name <= max & var_name >= min),
    qc_fail_min = sum(var_name < min),
    qc_fail_max = sum(var_name > max), 
    qc_null = sum(is.null(var_name))
  ))
...
vars_df <- vars_df %>% add_row(
  ...
  qc_null = c(sum(is.null(this_table$bmi_calculated)))
)
