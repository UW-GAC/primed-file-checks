...
vars_df <- vars_df %>% add_row(
  ...
  qc_fail_min = sum(this_table$bmi_1 < this_table$bmi_calculated - bmi_tolerance),
  qc_fail_max = sum(this_table$bmi_1 > this_table$bmi_calculated + bmi_tolerance),
  qc_null = sum(is.null(this_table$bmi_calculated))
)
