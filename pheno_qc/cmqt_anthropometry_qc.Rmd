...
df <- read.table(file = '/home/rstudio/primed-file-checks/pheno_qc/test_data/cmqt_anthropometry.tsv', sep = '\t', header = TRUE)
```
...
this_table <- this_table %>% mutate(
  bmi_consistency = ifelse(!is.na(weight_1) & !is.na(height_1), weight_1 / (height_1/100)^2, NULL)
  )
...
vars_df <- vars_df %>% add_row(
  ...
  min_values = NULL, 
  max_values = NULL, 
  qc_pass = sum(this_table$bmi_1 == this_table$bmi_consistency), 
  qc_fail_min = NULL, 
  qc_fail_max = NULL, 
  qc_null = c(sum(sapply(this_table$bmi_consistency,is.null)))
)
