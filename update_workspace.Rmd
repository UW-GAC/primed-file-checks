...
```{r}
original_bucket <- avstorage(namespace = "primed-cc-scratch", 
                            name = "PRIMED_example_genotype_1000G_test")
update_bucket <- avstorage(namespace = "primed-cc-scratch", 
...
for (f in files_to_move) {
  avcopy(f, sub(update_bucket, original_bucket, f))
}
```
...
```{r}
avcopy(paste0(update_bucket, "/notebooks/*"), paste0(original_bucket, "/notebooks/"), recursive=TRUE)
avcopy(paste0(update_bucket, "/submissions/*"), paste0(original_bucket, "/submissions/"), recursive=TRUE)
```
